...
function createAudio(content) {
    ...
    // build audio post if the source is from tumblr  
    if (content.provider === 'tumblr') {
        ...
        icons.classList.add('fa', 'fa-play')
        playButton.append(icons)
        ...
        // cover image
        let audioCover = document.createElement('div')
        audioCover.classList.add('audio-cover')
        let audioImage = document.createElement('img')
        if (content.poster) {
            audioWrapper.style.backgroundImage = `url(${content.poster[0].url})`
            audioWrapper.style.backgroundSize = 'cover'
            audioImage.src = content.poster[0].url
        }
        audioImage.classList.add('album-image')
        audioCover.append(audioImage)
        audioCover.append(playButton)

        // content/ info
        let audioContent = document.createElement('div')
        audioContent.classList.add('audio-content')
        audioContent.innerHTML = `<div><b>${content.title}</b><p>${content.artist}</p></div>`
        ...
        audio.ontimeupdate = () => {
            ...
            currentTime = convertSeconds(audio.currentTime)
            audioImage.style.transform = `rotate(${(percentagePosition / 100) * (360 * 2)}deg)`
            current.innerHTML = currentTime
        }
        ...
        audio.onloadedmetadata = () => {
            total = convertSeconds(audio.duration)
        ...
        audioContent.append(timeline, current, final)
        audioPost.append(audioCover, audioContent)
        audioWrapper.append(audioPost)

    }
    // embed if not from tumblr
    else {
...
for (const post of posts) {
 ...
 const totals = document.querySelectorAll('.featured-tags a[href*="/tagged"] .total')
    ...
    featuredTags.forEach((tag, index) => {
        let tagged = tag.href.split('/tagged/')[1]
        let tagData;
        let tagUrl = `https://{name}.tumblr.com/api/read/json?tagged=${ tagged }`
        function onErrors() {
        ...
        function onLoad() {
            ...
            if (data.posts[0]['photo-url-500'] != null) {
                let tagImg = document.createElement('img')
                tagImg.src = data.posts[0]['photo-url-500']
                placeholder.prepend(tagImg)
            }
            
            if (data.posts[0].type === 'regular') {
                placeholder.innerHTML = data.posts[0]['regular-body']
            }
    }
